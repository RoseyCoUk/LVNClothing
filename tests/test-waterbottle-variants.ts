import { waterbottleVariants, getWaterBottleVariantsByColor, getWaterBottleVariant, getWaterBottleColors, getWaterBottleSizes } from './hooks/waterbottle-variants';

// Test suite for water bottle variants
export function testWaterBottleVariants() {
  console.log('💧 Testing Water Bottle Variants System...\n');
  ...
  function testVariantCount() {
    ...
    const actualCount = waterbottleVariants.length;
  ...
  function testColorConsistency() {
    const colors = getWaterBottleColors();
  ...
  function testUniqueIds() {
    const internalIds = waterbottleVariants.map(v => v.id);
    const printfulIds = waterbottleVariants.map(v => v.printful_variant_id);
  ...
  function testPrintfulIdRange() {
    const minId = Math.min(...waterbottleVariants.map(v => v.printful_variant_id));
    const maxId = Math.max(...waterbottleVariants.map(v => v.printful_variant_id));
    
    const expectedMin = 8000;
    const expectedMax = 8099;
  ...
  function testPricingStructure() {
    ...
    for (const variant of waterbottleVariants) {
  ...
  function testStockStatus() {
    const allInStock = waterbottleVariants.every(v => v.in_stock === true);
  ...
  function testNamingConvention() {
    ...
    for (const variant of waterbottleVariants) {
      const expectedFormat = `${variant.color} Water Bottle - One Size`;
    ...
    testResults.push({
      ...
      passed,
      details: details || 'All variants follow "Color Water Bottle - One Size" format'
    });
  ...
  function testHelperFunctions() {
    ...
    // Test getWaterBottleColors
    const colors = getWaterBottleColors();
    if (colors.length !== 1) {
      passed = false;
      details = `getWaterBottleColors returned ${colors.length} colors, expected 1`;
    }
    
    // Test getWaterBottleSizes
    const sizes = getWaterBottleSizes();
    if (sizes.length !== 1 || sizes[0] !== 'One Size') {
      passed = false;
      details = `getWaterBottleSizes returned ${sizes.length} sizes, expected 1 with 'One Size'`;
    }
    
    // Test getWaterBottleVariant
    const testVariant = getWaterBottleVariant('White');
    if (!testVariant || testVariant.color !== 'White') {
      passed = false;
      details = 'getWaterBottleVariant failed to return correct variant';
    }
...
export default testWaterBottleVariants;
