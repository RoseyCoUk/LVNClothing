import { mousepadVariants, getMousePadVariantsByColor, getMousePadVariant, getMousePadColors, getMousePadSizes } from './hooks/mousepad-variants';

// Test suite for mouse pad variants
export function testMousePadVariants() {
  console.log('🖱️ Testing Mouse Pad Variants System...\n');
  ...
  function testVariantCount() {
    ...
    const actualCount = mousepadVariants.length;
  ...
  function testColorConsistency() {
    const colors = getMousePadColors();
  ...
  function testUniqueIds() {
    const internalIds = mousepadVariants.map(v => v.id);
    const printfulIds = mousepadVariants.map(v => v.printful_variant_id);
  ...
  function testPrintfulIdRange() {
    const minId = Math.min(...mousepadVariants.map(v => v.printful_variant_id));
    const maxId = Math.max(...mousepadVariants.map(v => v.printful_variant_id));
    
    const expectedMin = 9000;
    const expectedMax = 9099;
  ...
  function testPricingStructure() {
    ...
    for (const variant of mousepadVariants) {
  ...
  function testStockStatus() {
    const allInStock = mousepadVariants.every(v => v.in_stock === true);
  ...
  function testNamingConvention() {
    ...
    for (const variant of mousepadVariants) {
      const expectedFormat = `${variant.color} Mouse Pad - One Size`;
    ...
    testResults.push({
      ...
      passed,
      details: details || 'All variants follow "Color Mouse Pad - One Size" format'
    });
  ...
  function testHelperFunctions() {
    ...
    // Test getMousePadColors
    const colors = getMousePadColors();
    if (colors.length !== 1) {
      passed = false;
      details = `getMousePadColors returned ${colors.length} colors, expected 1`;
    }
    
    // Test getMousePadSizes
    const sizes = getMousePadSizes();
    if (sizes.length !== 1 || sizes[0] !== 'One Size') {
      passed = false;
      details = `getMousePadSizes returned ${sizes.length} sizes, expected 1 with 'One Size'`;
    }
    
    // Test getMousePadVariant
    const testVariant = getMousePadVariant('White');
    if (!testVariant || testVariant.color !== 'White') {
      passed = false;
      details = 'getMousePadVariant failed to return correct variant';
    }
...
export default testMousePadVariants;
