// Printful Safe Import Function - Import products without deleting existing data
import "jsr:@supabase/functions-js/edge-runtime.d.ts"
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2'

console.log("Printful Safe Import Function Started")
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('Starting safe Printful import...')
    ...
    // Transform Printful products to match your database schema
    const transformedProducts = products.map((product: PrintfulProduct) => ({
      ...
      slug: product.sync_product.name.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, ''),
      reviews: 0,
      rating: 0,
      price: parseFloat(product.sync_product.retail_price), // Use retail price as base price
      printful_cost: parseFloat(product.sync_product.retail_price) * 0.7, // Estimate cost
    ...
    if (newProducts.length > 0) {
      ...
      console.log(`Successfully inserted ${insertedProducts?.length || 0} new products`)
    }
    ...
    console.error('Error in Printful safe import function:', error)
