-- Phase 1: Database Schema & Printful Sync Foundation - FIX
-- This migration ensures the variant tables are created on the remote database

-- up

-- 1. Create enhanced product_variants table for Printful variants
...
-- 2. Create product_variant_combinations table for complex products (size + color)
...
-- 3. Create product_categories table for better organization
...
-- 6. Create RLS policies for product_variants
...
-- 7. Create RLS policies for product_variant_combinations
DROP POLICY IF EXISTS "Users can view available variant combinations" ON public.product_variant_combinations;
CREATE POLICY "Users can view available variant combinations" ON public.product_variant_combinations
  FOR SELECT USING (is_available = true);

DROP POLICY IF EXISTS "Authenticated users can manage variant combinations" ON public.product_variant_combinations;
CREATE POLICY "Authenticated users can manage variant combinations" ON public.product_variant_combinations
  FOR ALL USING (auth.role() = 'authenticated');

-- 8. Create RLS policies for product_categories
DROP POLICY IF EXISTS "Users can view active product categories" ON public.product_categories;
...
-- 11. Seed basic product categories
...
-- 12. Create function to get product with variants
