...
export async function getProductPrice(printfulVariantId: string): Promise<number> {
  ...
  // Parse the variant ID to give a reasonable estimate
  const variantId = parseInt(printfulVariantId, 10);
  
  if (variantId >= 10000 && variantId < 30000) {
    // T-shirt range - use consistent pricing, server handles size-based pricing
...
export async function validateAndEnrichCart(items: CartItem[]): Promise<CartItem[]> {
  ...
  for (const item of items) {
    try {
      ...
      enrichedItems.push({
        ...item,
        price: realPrice // Update with real-time price
      });
    } catch (error) {
      console.warn(`Failed to get price for item ${item.id}, using provided price`);
