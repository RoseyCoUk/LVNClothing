...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  return (
    <Link
      to={productUrl}
      className="bg-white rounded-2xl shadow-lg overflow-hidden flex flex-col transition-transform hover:-translate-y-1 hover:shadow-2xl duration-200 focus:outline-none focus:ring-2 focus:ring-[#009fe3] group w-full h-full aspect-square md:aspect-auto"
      aria-label={`View details for ${product.name}`}
    >
      {/* Product Image */}
      <div className="bg-gray-100 aspect-square w-full flex items-center justify-center md:aspect-auto md:h-64">
        {product.image_url ? (
          <img 
            src={product.image_url} 
            alt={product.name} 
            className="object-cover w-full h-full group-hover:scale-105 transition-transform duration-200 aspect-square md:aspect-auto md:h-64" 
            onError={(e) => {
        ...
        <div className={`w-full h-full flex items-center justify-center text-gray-400 md:h-64 ${product.image_url ? 'hidden' : ''}`}>
