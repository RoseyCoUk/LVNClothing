  ...
  if (shippingError) {
    return (
      <div className={`text-red-600 bg-red-50 border border-red-200 rounded-lg p-4 ${className}`}>
        ...
        <p className="mt-2 text-sm">{shippingError}</p>
      </div>
    )
  }
    ...
    <div className={className}>
      ...
      <div className="space-y-3">
        {shippingOptions.map((option) => {
          ...
          const deliveryDays = option.minDeliveryDays && option.maxDeliveryDays
            ? option.minDeliveryDays === option.maxDeliveryDays
              ? `${option.minDeliveryDays} day${option.minDeliveryDays !== 1 ? 's' : ''}`
            ...
            >
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  {option.carrier && (
                    <div className="text-xs text-gray-400">{option.carrier}</div>
                  )}
                </div>
              </div>
              ...
              <div className="text-right">
                <div className="font-medium text-gray-900">
                  {formatCurrency(parseFloat(option.rate), option.currency)}
                </div>
              </div>
            </label>
          )
