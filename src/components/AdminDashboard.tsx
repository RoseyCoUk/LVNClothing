...
const AdminDashboard: React.FC = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Calculate stats (only real orders, exclude test orders with £0.00)
      const realOrders = ordersData?.filter(order => (order.amount_total || 0) > 0) || [];
          ...
          <div className="flex justify-between items-center py-4">
            <div>
              ...
              <p className="text-sm text-gray-600">
                Welcome back, {user?.email} • Test orders (£0.00) excluded from statistics
              </p>
            </div>
            <button
